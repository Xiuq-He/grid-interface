...
T_SC = 0.2 * s;                 % Specification of SC response (0.0 - 0.4s)
                                % Note that this time constant should be 
                                % less than C_Inertia ideally

% Disaggregation (for I. DC grid-supporting implementation)
% Control objectives of the DC-DC boost converters
T_SC1 = T_SC/(0.01*s + 1);                       % Causalized transfer function to be implemented
T_agg = 1/(T_des_vdcf*T_des_fp) - 1/2*C_pu*s/w0 - T_SC; % (- T_SC1) or (- T_SC)
T_PV = T_agg * 0.7/(0.5*s + 1);                  % Slow PV (T_PV is already causal)
T_ESS = T_agg - T_PV;                            % Fast ESS
T_ESS1 = T_ESS/(0.01*s + 1)/(0.01*s+1);          % Causalized transfer function to be implemented
...
figure;
subplot(2,1,1);
bodemag(T_des); hold on
bodemag(T_SC1); bodemag(T_PV); bodemag(T_ESS1)
subplot(2,1,2);
bodemag(T_des_fp); hold on
bodemag(T_des_fp_agg_1);

% Disaggregation (for II. DC grid-forming implementation)
% Control objectives of the DC-DC boost converters
T_inv_SC = 1/T_SC;
T_agg = 1/(T_des_vdcf*T_des_fp) - 1/2*C_pu*s/w0 - T_SC;
T_PV = T_agg * 0.7/(0.5*s + 1);                  % Slow PV
T_inv_PV = 1/T_PV * 1/(0.01*s + 1);              % TF to be implemented
T_ESS = T_agg - T_PV;                            % Fast ESS
T_inv_ESS = 1/T_ESS;
...
% (re-aggregated dynamics with the causalized desired control responses)
T_des = 1/(T_des_vdcf*T_des_fp);
T_des_agg = T_SC + 1/T_inv_PV + 1/T_inv_ESS + 1/2*C_pu*s/w0;
T_des_fp_agg_2 = 1/(T_des_vdcf*T_des_agg);

figure;
subplot(2,1,1);
bodemag(T_des); hold on
bodemag(T_SC); bodemag(1/T_inv_PV); bodemag(1/T_inv_ESS)
subplot(2,1,2);
bodemag(T_des_fp); hold on
bodemag(T_des_fp_agg_2);
